...
| `three-nodes` | Docker attacker/web/db 实验拓扑 |
...
Docker 节点可以在 provider 块中覆盖 OCI 镜像的 `ENTRYPOINT` 和 `CMD`：
...
省略属性会继承镜像值，非空数组会替换镜像值，显式 `[]` 会清空镜像值。两项都使用直接 argv 语义，Sysbox 不插入 shell。这些字段只属于 Docker provider；libvirt 和 Firecracker 使用各自的启动配置。
